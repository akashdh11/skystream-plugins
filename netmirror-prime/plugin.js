(function() {
    ...
    async function getHome(cb) {
        try {
            ...
            const res = await http_get(`${PLAY_URL}/pv/homepage.php`, { ...CommonHeaders, "Referer": `${BASE_URL}/home`, "Cookie": cookieStr });
    ...
    async function load(urlData, cb) {
        try {
            ...
            const cookieStr = await getCookieString();
            const url = `${PLAY_URL}/pv/post.php?id=${id}&t=${Math.floor(Date.now()/1000)}`;
            ...
            if (data.episodes && data.episodes.length > 0 && data.episodes[0]) {
                data.episodes.forEach(ep => {
                    episodes.push(new Episode({
                        ...
                        url: JSON.stringify({ id: ep.id, title: ep.t }), posterUrl: `https://imgcdn.kim/pvepimg/150/${ep.id}.jpg`
                    }));
                });
    ...
    async function fetchEpisodes(seriesId, seasonId, page, episodes, cookieStr) {
        ...
        while (true) {
            try {
                const url = `${PLAY_URL}/pv/episodes.php?s=${seasonId}&series=${seriesId}&t=${Math.floor(Date.now()/1000)}&page=${pg}`;
