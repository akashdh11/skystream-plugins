        ...
        try {
            ...
            const extractSectionLinks = (titleText) => {
                ...
                for (const link of links) {
                    ...
                    if (title && url && url.includes('topic') && !url.includes('profile') && title.length > 8) {
            ...
            const sidebarTitles = doc.querySelectorAll('.ipsWidget_title, .ipsWidget_title span, h3');
            let weekWidget = null;
            for (const t of sidebarTitles) {
                const text = t.textContent.trim().toUpperCase();
                if (text.includes("WEEK") && text.includes("RELEASE")) {
            ...
            if (weekWidget) {
                const topics = weekWidget.querySelectorAll('.ipsDataItem_title a');
                const items = [];
