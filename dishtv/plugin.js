(function() {
    ...
    const M3U_URL = "https://alex4528.site/playlist/dishtv.m3u";
    ...
    async function fetchM3U() {
        ...
        const response = await http_get(M3U_URL, headers);
        const status = response.status !== undefined ? response.status : response.statusCode;
        if (status >= 200 && status < 300) {
            return response.body.trim();
        } else {
            throw new Error(`HTTP Error ${status || 'No Response'} fetching Dish TV M3U`);
        }
    }
    ...
    function parseM3U(m3uString) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith("#KODIPROP:inputstream.adaptive.license_key=")) {
                pendingKodiProps.licenseUrl = line.split("=")[1].trim();
            } else if (line.startsWith("#KODIPROP:inputstream.adaptive.license_type=")) {
                pendingKodiProps.licenseType = line.split("=")[1].trim();
                ...
                currentChannel = {
                    ...
                    headers: {},
                    kodiProps: JSON.parse(JSON.stringify(pendingKodiProps))
                };
                pendingKodiProps = {}; 
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            if (channel.kodiProps && channel.kodiProps.licenseUrl) {
                const lp = channel.kodiProps.licenseUrl;
                if (lp.split(":").length === 2 && !lp.includes("http")) {
                    stream.drmKid = lp.split(":")[0];
                    stream.drmKey = lp.split(":")[1];
                } else {
                    stream.licenseUrl = lp;
                }
            }
