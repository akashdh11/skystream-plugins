(function() {
    ...
    async function getHome(cb) {
        try {
            ...
            if (!res || !res.body) return cb({ success: false, errorCode: "SITE_OFFLINE", message: "Failed to load playlist" });
            cb({ success: true, data: parseM3uToCarousels(res.body) });
        } catch (e) {
    ...
    async function search(query, cb) {
        try {
            ...
            const cats = parseM3uToCarousels(res.body);
            const results = [];
